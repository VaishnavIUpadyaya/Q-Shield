...
from quantum.protocol import sign_document_payload, verify_document_payload
...
async def hash_document_endpoint(
    ...
    """
    Compute cryptographic SHA-256 digest and partition into 128 2-bit quantum blocks.
    """
    try:
        ...
        if "application/json" in content_type:
            ...
            if body.get("document_base64"):
                content = base64.b64decode(body["document_base64"])
            elif body.get("document_text"):
                content = body["document_text"].encode("utf-8")
            ...
            try:
                ...
                if "file" in form and form["file"]:
                    ...
                    content = str(form["document_text"]).encode("utf-8")
                elif "document_base64" in form:
                    content = base64.b64decode(str(form["document_base64"]))
            except Exception:
                pass
        ...
        if content is None:
            raise HTTPException(
                ...
                detail="Must provide a file upload, document_text, or document_base64",
            )

        doc_hash, chunks = hash_and_chunk_document(content)
...
@router.post(
    ...
    summary="Sign a document payload using Xu-Wang QDS across 128 quantum blocks",
)
async def sign_document(
    ...
    """
    Compute the SHA-256 digest of an uploaded file or text, partition it into 128 2-bit
    quantum blocks, and sign each chunk using the Xu-Wang QDS protocol with Table 1 private keys.
    """
    try:
        content_type = request.headers.get("content-type", "")
        actual_signer = "Alice"
        ...
        if "application/json" in content_type:
            ...
            if req.document_base64:
                content = base64.b64decode(req.document_base64)
            elif req.document_text:
                content = req.document_text.encode("utf-8")
            elif req.document_hash:
                content = req.document_hash
            else:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Must provide document_base64, document_text, or document_hash",
                )
            ...
            try:
                ...
                if req.document_base64:
                    ...
                    raise ValueError("No valid document field in body")
            except Exception:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="No document payload provided for signing",
                )

        signature = sign_document_payload(content, signer_id=actual_signer)
...
@router.post(
    ...
    summary="Measure and verify multi-block QDS state projections against a document",
)
async def verify_document(
    ...
    """
    Verify a multi-block quantum seal against a document or document hash.
    Simulates Bob's two-qubit measurement circuits for all 128 blocks and detects tampering.
    """
    try:
        content_type = request.headers.get("content-type", "")
        ...
        if "application/json" in content_type:
            ...
            if req.document_base64:
                content_to_verify = base64.b64decode(req.document_base64)
            elif req.document_text:
                content_to_verify = req.document_text.encode("utf-8")
            elif req.document_hash:
                content_to_verify = req.document_hash
            else:
                content_to_verify = sig_data.get("document_hash", "")
                if not content_to_verify:
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail="Must provide document_base64, document_text, document_hash, or file",
                    )
        elif "multipart/form-data" in content_type or file is not None:
            ...
            if file is None or not raw_sig:
                ...
                file_item = form.get("file")
                if file_item is not None and hasattr(file_item, "read"):
                    content_to_verify = await file_item.read()
                if form.get("signature_json"):
                    raw_sig = str(form.get("signature_json"))
                if form.get("shots"):
                    actual_shots = int(form.get("shots"))
            else:
            ...
            if not raw_sig:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="signature_json is required when uploading a file for verification",
                )
            try:
                sig_data = json.loads(raw_sig) if isinstance(raw_sig, str) else raw_sig
            except Exception as e:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Invalid JSON in signature_json: {str(e)}",
                )
        else:
            try:
                body = await request.json()
        ...
        status_str = (
            ...
            else ("tampered" if verification_result.tampered else "rejected")
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Document verification failed: {str(exc)}",
        )
